...
COMMENT ON COLUMN BBN_ORDER.ORDER_ID IS '주문번호';
COMMENT ON COLUMN BBN_ORDER.ID IS '회원아이디';
COMMENT ON COLUMN BBN_ORDER.WINE_BNO IS '와인게시물번호';
COMMENT ON COLUMN BBN_ORDER.CART_ID IS '장바구니번호';
COMMENT ON COLUMN BBN_ORDER.ORDER_DATE IS '주문시간';

 ALTER TABLE BBN_ORDER ADD CONSTRAINT BBN_ORDER_FK_CART_ID FOREIGN KEY (CART_ID) REFERENCES CART(CART_ID);
 
 ALTER TABLE BBN_ORDER ADD CONSTRAINT BBN_ORDER_FK_ID FOREIGN KEY (ID) REFERENCES MEMBER(ID); 
 
 ALTER TABLE BBN_ORDER ADD CONSTRAINT BBN_ORDER_FK_WINE_BNO FOREIGN KEY (WINE_BNO) REFERENCES BEBENO_WINEBOARD(WINE_BNO); 
